...
const logs = require("../src/logger.js");
...
const DocumentHandler = function (options) {
  ...
  if (this.createKey !== "") {
    logs.auth("Creation-Key:", this.createKey);
  }
};
...
DocumentHandler.prototype.handleRawGet = function (key, res) {
  this.store.get(key, function (ret) {
    if (ret) {
      logs.success("Open paste:", key);
      res.writeHead(200, { "content-type": "text/plain" });
      res.end(ret);
    } else {
      logs.error("Paste not found:", key);
      res.writeHead(404, { "content-type": "application/json" });
      res.end(JSON.stringify({ message: "Paste not found." }));
    }
  });
...
DocumentHandler.prototype.handlePost = function (req, res) {
  ...
  req.on("data", function (data) {
    ...
    if (_this.maxLength && buffer.length > _this.maxLength) {
      cancelled = true;
      logs.error("Paste exeeds maximum length.");
  ...
  req.on("end", function () {
    ...
    if (_this.createKey !== "") {
      if (!buffer.startsWith(_this.createKey)) {
        logs.error("Error adding new paste: wrong key");
        res.writeHead(400, { "content-type": "application/json" });
    ...
    _this.chooseKey(function (key) {
      _this.store.set(key, buffer, function (success) {
        if (success) {
          ...
          logs.error("Error adding new paste.");
